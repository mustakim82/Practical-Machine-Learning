...
There is a large number of variables that can be considered IDs, for instance **timestamp** and **user name**. In my opinion they should not be used to model, the objective is to use the measures in the data set to predict the quality of the fisical exercises.
...
Using this map we can see there is a lot of variables with many missing instances, let's remove the features with many missing values (> 70%).
...
There is two variables that are related to the time stamp. I chose to remove **new_window** and **num_window**, they seem more of an artifact of the data collection method then a true feature for this classification problem. We can always try to use them if the final modeling results are weak.
...
Now we have a vector of the variables we want to **remove**, so let's clean our data...
...
Now it looks very clean, let's look again at the structure of this data set.
...
Let's try to look at the most important features:
...
Very nice results on the testing data set. Let's look at the confusion matrix.

```{r confusionmatrix}
confusionMatrix(test.predict, testing.clean$classe)
```

We got ourselves a very accurate model, on the training and test dataset.

## The prediction to submit.

Let's get and clean the prediction data set.
